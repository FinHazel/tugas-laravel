...
<html lang="en">
  ...
  <body>
    ...
    <div style="float:right;">
         <form class="" action="/cari_apple" method="GET">
  ...
  <tbody>
    <tr>
      <th scope="row">{{$apple->firstItem() + $loop->index}}</th>
      <td>{{$a->type}}</td>
      <td>{{$a->harga}}</td>
      <td>{{$a->spesifikasi}}</td>
      <td>
      <div class="btn-group" role="group" aria-label="basic mixed styles example">
      <form action="/apple/{{ $a->id }}" method="post">
        @method('delete')
        @csrf
        <button type="submit" onclick="return confirm('Yakin Mau Menghapus Data?')" class="btn btn-danger">Hapus</button>
        <a href="/edit_apple/{{$a->id}}" class="btn btn-warning">Edit</a>
        <a href="/lihatapple/{{$a->id}}" class="btn btn-success">Lihat</a>
      </form>
      </div>
      </td>
    </tr>
